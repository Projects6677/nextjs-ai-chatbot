...
export async function voteMessage({
  ...
  try {
    ...
    if (existingVote) {
      return await db.update(vote).set({ isUpvoted: type === 'up' }).where(and(eq(vote.messageId, messageId), eq(vote.chatId, chatId)));
    }
...
export async function getSuggestionsByDocumentId({ documentId }: { documentId: string; }) {
  try {
    return await db.select().from(suggestion).where(and(eq(suggestion.documentId, documentId)));
  } catch (error) {
...
// --------- Example Query Functions for userProfile and careerRecommendation ----------
...
export async function getUserProfileByUserId(userId: string) {
  try {
    ...
    throw new ChatSDKError('bad_request:database', 'Failed to get user profile');
  }
}

// Get career recommendations for a user
export async function getCareerRecommendationsByUserId(userId: string) {
  try {
    return await db.select().from(careerRecommendation).where(eq(careerRecommendation.userId, userId));
  } catch (error) {
    throw new ChatSDKError('bad_request:database', 'Failed to get career recommendations');
  }
}
